export const LineChart = ({ series, strokeColor = '#0078d4', fillColor = 'rgba(0, 120, 212, 0.14)', height = 240 }) => {
  ...
  return (
    <div className="space-y-4">
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full" style={{ height }}>
        ...
        <polyline fill="none" stroke={strokeColor} strokeWidth="2.2" points={linePath} />
        {coordinates.map((point) => (
          <circle key={point.label} cx={point.x} cy={point.y} r="1.8" fill={strokeColor} />
        ))}
      </svg>
      <div className="grid grid-cols-6 gap-2 text-xs text-[var(--text-secondary)] md:grid-cols-12">
        {series.map((point) => (
